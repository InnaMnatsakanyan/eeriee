...
export default function TrackPreview({ track, onPlay }: TrackPreviewProps) {
    ...
    return (
        <div className={styles.similarTrackCell}>
            ...
            <p className={styles.trackArtistName}>{track.artist_name}</p>
            <p className={styles.trackAlbumName}>{track.album_name}</p>
            <p className={styles.trackDuration}>{durationUIMapper.unixToMinuteSecond(track.duration)}</p>
            ...
            {/*play button*/}
            <button className={styles.currentTrackButtons} onClick={onPlay}>
                <svg width="24px"
                     height="24px"
                     viewBox="0 0 24 24"
                     fill="none"
